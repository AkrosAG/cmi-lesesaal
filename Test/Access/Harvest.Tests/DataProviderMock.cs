...
namespace CMI.Access.Harvest.Tests
{
    internal class DataProviderMock
    {
        ...
        public IAISDataProvider GetMock()
        {
            ...
            //moq.Setup(t => t.LoadDetailData(1000).Result).Returns(detailDataSet);
            //moq.Setup(t => t.LoadContainers(1000).Result).Returns(containerDataSet);
            //moq.Setup(t => t.GetArchiveRecordRow(1000).Result).Returns((ArchiveRecordDataSet.ArchiveRecordRow) archiveRecordDataSet.ArchiveRecord.Rows[0]);
            //moq.Setup(t => t.LoadNodeContext(1000).Result)
            ...
            //    });
            //moq.Setup(t => t.LoadNodeInfo(1000).Result).Returns(nodeInfoDataSet);
            //moq.Setup(t => t.LoadDescriptors(1000).Result).Returns(descriptorDataSet);
            //moq.Setup(t => t.LoadReferences(1000).Result).Returns(referencesDataSet);
            //moq.Setup(t => t.LoadArchivePlanInfo(new[] {1L, 800L, 1000L}).Result).Returns(archivePlanInfoDataSet);
            //moq.Setup(t => t.LoadMetadataSecurityTokens(1000).Result).Returns(new List<string> {"Ö1"});
            //moq.Setup(t => t.LoadPrimaryDataSecurityTokens(1000).Result)
