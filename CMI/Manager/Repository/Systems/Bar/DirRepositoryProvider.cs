...
public class DirRepositoryProvider: IRepositoryProvider
{
    ...
    /// <summary>
    /// Initializes the Repository Provider for the BAR DIR system
    /// </summary>
    /// <param name="dirRepositoryDataAccess">The repository data access.</param>
    /// <param name="dirPackageValidator">A validation engine to validate the package.</param>
    ...
    public DirRepositoryProvider(IDirRepositoryDataAccess dirRepositoryDataAccess, IDirPackageValidator dirPackageValidator, IPackageHandler handler,
        IParameterHelper parameterHelper, IBus bus)
    {
        this.dirRepositoryDataAccess = dirRepositoryDataAccess;
        ...
        try
        {
            ...
            if (rootFolder != null)
            {
                ...
                try
                {
                    ...
                    dirPackageValidator.EnsureValidPhysicalFileAndFolderNames(retVal.PackageDetails,
    ...
    public async Task<RepositoryPackageInfoResult> ReadPackageMetadata(string packageId, string archiveRecordId)
    {
        ...
        try
        {
            ...
            if (rootFolder != null)
            {
                ...
                retVal.PackageDetails.FileCount = GetFileCountFromMetadata(retVal.PackageDetails, false); // Get all files according to DIR metadata
    ...
    private void SaveFiles(string folderName, List<RepositoryFile> files, List<string> fileTypesToIgnore)
    {
        foreach (var file in files)
        {
            try
            {
                ...
                using (var stream = dirRepositoryDataAccess.GetFileContent(file.Id))
                {
    ...
    {
        // Get the sub folders
        folder.Folders = dirRepositoryDataAccess.GetFolders(folder.Id);

        // Get the files
        folder.Files = dirRepositoryDataAccess.GetFiles(folder.Id, ignoredFilenameRegex, out var ignored);
