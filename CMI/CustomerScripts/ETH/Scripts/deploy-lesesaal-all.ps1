...
param(
    ...
    [bool]$ServiceStart = $false        <# Service wird automatisch gestartet nach der Installation #>
)
...
Function Main
{
    ...
    $ServicesRootDir =  [IO.Path]::Combine($ServicesRootDir,$Instance)
    if ($false -eq [System.IO.Directory]::Exists($ServicesRootDir))
    {
       Write-Output "Das angegebene ServicesRootDir wurde nicht gefunden und wird angelegt." 
       New-Item -ItemType Directory -Force -Path  $ServicesRootDir
    }

    Write-Output "Service Installationsverzeichnis ist $ServicesRootDir."

    $WebSitesDir =  [IO.Path]::Combine($WebSitesDir,$Instance)
    if ($false -eq [System.IO.Directory]::Exists($WebSitesDir))
    {
       Write-Output "Das angegebene WebSitetDir wurde nicht gefunden und wird angelegt." 
       New-Item -ItemType Directory -Force -Path  $WebSitesDir
    }

    Write-Output "WebSite Installationsverzeichnis ist $WebSitesDir."
...
Function Install-Services
{
    ...
    $ServicesToInstall | ForEach-Object {
        Install-Service -ServiceToInstall $_
    }
}
...
Function Install-Service
{
    ...
    $serviceId = "CMI${ServiceToInstall}Service" + '$' + "${Instance}"
    ...
    {
        #Parameter file
          Write-Host "Urban Code wird ausgefuehrt" -ForegroundColor Yellow
...
Function Register-Service 
{
    ...
    & "$pathToServiceExe" install -instance ${Instance}
...
Function Backup-Service
{
    ...
    if ([System.IO.Directory]::Exists($source))
    {
        ...
        $destination = [System.Io.Path]::Combine($BackupDir,"$Instance","$ServiceToBackup", $date)
...
Function Backup-Web
{
    ...
    {
        $date = Get-Date -Format "yyyy-MM-dd HH.mm.ss"
        $destination = [System.Io.Path]::Combine($BackupDir,"$Instance", "$web", $date)
        Write-Output "Erstelle Backup von $source -> $destination"
        Copy-Item -Path $source -Destination $destination -Recurse -Container
        Write-Output "Backup Beendet"
    }
