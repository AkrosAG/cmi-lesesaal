...
namespace CMI.Access.Harvest.ScopeArchiv
{
    ...
    public class ScopeAISDataProvider : IAISDataProvider
    {
        ...
        {
            var ds = await GetDataSetFromSql<DataSet>(SqlStatements.SqlMutationsRecords);
        ...
        public async Task<int> UpdateMutationStatus(MutationStatusInfo info)
        {
            ...
            using (var cn = AISConnection.GetConnection())
            {
                ...
                {
                    await InsertMutationStatusLog(info);
                }
            }
            ...
            using (var cn = AISConnection.GetConnection())
            {
                ...
                try
                {
                    ...
                    using (var cmd = new OracleCommand(sql.ToString(), cn))
                    {
                        ...
                        affectedRecords = await Task.Run(() => cmd.ExecuteArray(infos.Count));
                    }
        ...
        {
            return await ExecuteSql(SqlStatements.ResetFailedOperations, new[]
            {
        ...
        private async Task<int> InsertMutationStatusLog(MutationStatusInfo info)
        {
            ...
            return await ExecuteSql(SqlStatements.SqlUpdateMutationActionLog, new[]
            {
        ...
        public async Task<ArchiveRecordDataSet.ArchiveRecordRow> GetArchiveRecordRow(long recordId)
        {
            ...
            {
                Console.Out.WriteLine($"Should have received a record for id {recordId}");
            }
...
/// <summary>
        ...
        public async Task<ArchivePlanInfoDataSet> LoadArchivePlanInfo(long[] recordIdList)
        {
            var ids = string.Join(",", recordIdList);
            return await GetDataSetFromSql<ArchivePlanInfoDataSet>(string.Format(SqlStatements.SqlArchivePlanInfo, ids));
        }
        ...
        public async Task<int> InitiateFullResync()
        {
            try
            {
                return await ExecuteSql(SqlStatements.InitiateFullResync);
            }
            catch (Exception ex)
            {
                Log.Error(ex, ex.Message);
        ...
        public async Task<HarvestStatusInfo> GetHarvestStatusInfo(QueryDateRange dataRange)
        {
            ...
            try
            {
                var ds = await GetDataSetFromSql<DataSet>(SqlStatements.HarvestStatusInfo, new[]
                {
        ...
        public async Task<HarvestLogInfoResult> GetHarvestLogInfo(HarvestLogInfoRequest request)
        {
            ...
            using (var cn = AISConnection.GetConnection())
            {
                ...
                if (ds.Tables["LogInfo"].Rows.Count > 0)
                {
                    ...
                    dsDetail = await GetDataSetFromSql<DataSet>(string.Format(SqlStatements.HarvestLogInfoDetail, ids));
                }
        ...
        {
            var ds = await GetDataSetFromSql<DataSet>(SqlStatements.FondsOverviewList);
        ...
        public async Task<DataRow> GetLinkedAccessionToArchiveRecord(long recordId)
        {
            ...
            if (table != null && table.Rows.Count > 0)
            {
                ...
                var ds = await GetDataSetFromSql<AccessionDataSet>(SqlStatements.GetAccession, new[]
                {
        ...
        {
            return await ExecuteSqlScalar<string>("select optn_vchr from tba_optn where optn_id = :optn_id", new[]
            {
        ...
        private async Task<T> GetDataSetFromSql<T>(string sql, OracleParameter[] parameters = null) where T : DataSet, new()
        {
            var ds = new T();
            using (var cn = AISConnection.GetConnection())
            {
        ...
        /// <returns>The number of affected records</returns>
        private async Task<int> ExecuteSql(string sql, OracleParameter[] parameters = null)
        {
            using (var cn = AISConnection.GetConnection())
            {
                var sw = new Stopwatch();
        ...
        /// <param name="parameters">The parameters.</param>
        /// <returns>The number of affected records</returns>
        private async Task<T> ExecuteSqlScalar<T>(string sql, OracleParameter[] parameters = null)
        {
            using (var cn = AISConnection.GetConnection())
            {
                var sw = new Stopwatch();
